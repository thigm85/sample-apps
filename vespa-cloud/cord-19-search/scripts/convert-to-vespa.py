...
def produce_vespa_json(idx, row):
    ...
    try:
        timestamp = int(
            datetime.datetime.strptime(publish_time, "%Y-%m-%d").timestamp()
        )
    except:
        pass
    ...
    if has_full_text:
        authors, abstract_paragraphs, body_paragraphs, bib_entries, abstract, body = parse_file(
            full_text_dir, sha
        )
    else:
    ...
    vespa_doc = {
        ...
        "fields": {
            ...
            "body_embedding": {"values": None},
        },
    }
    if abstract:
        abstract_embedding = model.encode([abstract])[0]
        vespa_doc["fields"]["abstract_embedding"][
            "values"
    ...
    if body:
        ...
        vespa_doc["fields"]["body_embedding"]["values"] = body_embedding.tolist()
    else:
        vespa_doc["fields"].pop("body_embedding", None)
